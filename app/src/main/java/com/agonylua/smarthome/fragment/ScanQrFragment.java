...
import com.agonylua.smarthome.utils.PermissionUtils;
        ...
        mCodeScanner.setDecodeCallback(new DecodeCallback() {
            ...
            public void onDecoded(@NonNull final Result result) {
                requireActivity().runOnUiThread(() -> {
                    ...
                    // 1. 先重置 ViewModel 中的历史状态
                    viewModel.resetState();

                    // 2. 将数据交给 ViewModel，由其异步发起后端绑定请求
                    viewModel.parseQrCode(qrData);

                    // 3. 【核心改动】不论 API 结果如何，立即跳转到配网页面，
                    // 页面会展现加载状态并提示后端绑定进度
                    Navigation.findNavController(view).navigate(R.id.provisionFragment);
                });
            }
        });
